...
use utils::{
    active_id, attr_or_empty, click, click_nth, count, exists, focus, input, keydown, mount_root,
...
async fn tabs_select_hide_and_keyboard() {
    ...
    assert_eq!(attr_or_empty(&tab("one"), "aria-selected"), "true");
    assert_eq!(attr_or_empty(&tab("one"), "tabindex"), "0");
    assert_eq!(attr_or_empty(&tab("two"), "aria-selected"), "false");
    assert_eq!(attr_or_empty(&tab("two"), "tabindex"), "-1");
    assert!(!query(&panel("one")).has_attribute("hidden"));
    assert!(query(&panel("two")).has_attribute("hidden"));
    assert_eq!(attr_or_empty(&panel("two"), "data-state"), "inactive");

    // Trigger and panel are linked both ways.
    let controls = attr_or_empty(&tab("two"), "aria-controls");
    assert_eq!(attr_or_empty(&panel("two"), "id"), controls);
    assert_eq!(
        attr_or_empty(&panel("two"), "aria-labelledby"),
        attr_or_empty(&tab("two"), "id")
    );

    click(&tab("two"));
    settle().await;
    assert_eq!(attr_or_empty(&tab("two"), "aria-selected"), "true");
    assert!(query(&panel("one")).has_attribute("hidden"));
    assert!(!query(&panel("two")).has_attribute("hidden"));
    ...
    settle().await;
    assert_eq!(attr_or_empty(&tab("three"), "aria-selected"), "true");
    assert_eq!(active_id(), attr_or_empty(&tab("three"), "id"));
...
async fn menubar_open_close_and_radio() {
    ...
    assert_eq!(attr_or_empty(FILE_TRIGGER, "aria-expanded"), "false");

    // Click toggles open.
    click(FILE_TRIGGER);
    settle().await;
    assert_eq!(attr_or_empty(FILE_TRIGGER, "aria-expanded"), "true");
    ...
    settle().await;
    assert_eq!(attr_or_empty(FILE_TRIGGER, "aria-expanded"), "false");
    assert_eq!(attr_or_empty(PROFILES_TRIGGER, "aria-expanded"), "true");
    assert_eq!(count("#menubar-test .menubar-content"), 1);
    ...
    assert_eq!(attr_or_empty(&radio("a"), "aria-checked"), "true");
    click(&radio("b"));
    settle().await;
    assert_eq!(text("#menubar-test #menubar-profile"), "b");
    assert_eq!(attr_or_empty(&radio("b"), "aria-checked"), "true");
    assert_eq!(attr_or_empty(&radio("a"), "aria-checked"), "false");
    ...
    settle().await;
    assert_eq!(count("#menubar-test .menubar-content"), 0);
    assert_eq!(active_id(), attr_or_empty(FILE_TRIGGER, "id"));
...
async fn navigation_menu_shows_only_active_content() {
    ...
    assert_eq!(attr_or_empty(docs, "aria-expanded"), "false");
    ...
    assert_eq!(attr_or_empty(docs, "aria-expanded"), "true");
    assert_eq!(
        attr_or_empty(docs, "aria-controls"),
        attr_or_empty("#nav-test .navigation-menu-content", "id")
    );
...
async fn combobox_filters_selects_and_closes() {
    ...
    assert_eq!(
        attr_or_empty("#combobox-test .combobox-trigger", "aria-expanded"),
        "false"
    );
...
async fn dropdown_radio_items_follow_group_value() {
    ...
    assert_eq!(attr_or_empty(&item("top"), "aria-checked"), "true");
    assert_eq!(attr_or_empty(&item("bottom"), "aria-checked"), "false");

    click(&item("bottom"));
    settle().await;
    assert_eq!(text("#dropdown-test #dropdown-position"), "bottom");
    assert_eq!(attr_or_empty(&item("bottom"), "aria-checked"), "true");
    assert_eq!(attr_or_empty(&item("top"), "aria-checked"), "false");
}
...
async fn sidebar_follows_provider_open_state() {
    ...
    assert_eq!(
        attr_or_empty("#sidebar-test aside.sidebar", "data-state"),
        "expanded"
    );
